...
def cli(verbose, path):
    """Tidy a directory"""
    downloads_path = path or os.path.join(os.path.expanduser("~"), "Downloads")

    file_mappings = collections.defaultdict()
    for filename in os.listdir(downloads_path):
        if os.path.isfile(os.path.join(downloads_path, filename)) and not filename.startswith("."):
    ...
    for folder_name, folder_items in file_mappings.items():
        ...
        if verbose:
            click.echo(f" Tidying {len(folder_items)} files in: {downloads_path} ".center(100, "="))
